...
function renderTabs(keepTab) {
  ...
  allowed.forEach(tab => {
    ...
    // Create empty panel — content built on first switchTab (lazy)
    const panel = document.createElement('div');
    panel.className='tab-content';
    panel.id='panel-'+tab.id;
    panel.dataset.built = '0'; // flag: not yet built
    panels.appendChild(panel);
  });

  // Profile panel — also lazy
  const profileTab = state.tabs.find(t=>t.isProfile);
  if(profileTab && currentUser.tabs.includes('profile')) {
    const panel = document.createElement('div');
    panel.className='tab-content';
    panel.id='panel-profile';
...
function _buildPanelContent(tab, panel) {
  if(tab.isFrontPage)   { panel.innerHTML = buildFrontPagePanel(); refreshFrontPagePanel(); }
  else if(tab.isLoreHub){ panel.innerHTML = buildLoreHubPanel();   refreshLoreHubPanel(); }
  else if(tab.isAdmin)  { panel.innerHTML = buildAdminPanel();     refreshAdminPanel(); }
  else if(tab.isMod)    { panel.innerHTML = buildModPanel(); }
  else if(tab.isHacker) { panel.innerHTML = buildHackerPanel();    refreshHackerPanel(); }
  else if(tab.isProfile){ panel.innerHTML = buildProfilePanel();   refreshProfilePanel(); }
  else if(tab.isChat)   { panel.innerHTML = buildChatPanel();      refreshChatPanel(); }
  else if(tab.isCamera) { panel.innerHTML = buildCameraPanel();    refreshCameraPanel(); }
  else if(tab.isNewspaper) panel.innerHTML = buildNewspaperPanel(tab);
  else if(tab.isSysNews)   panel.innerHTML = buildSysNewsPanel(tab);
  else if(tab.isRumors)    panel.innerHTML = buildRumorsPanel(tab);
  else if(tab.isMarket) { panel.innerHTML = buildMarketPanel(tab); refreshMarketPanel(tab); }
  else if(tab.isIdent)  { panel.innerHTML = buildIdentPanel(tab);  refreshIdentPanel(tab); }
  else if(tab.isWanted) { panel.innerHTML = buildWantedPanel(tab); refreshWantedPanel(tab); }
  else if(tab.isHub)    { panel.innerHTML = buildHubPanel(tab);    refreshHubPanel(tab); }
  else                    panel.innerHTML = buildContentPanel(tab);
...
function switchTab(id) {
  ...
  // ── Lazy build: first time this panel is visited ──────────────────
  if(panel.dataset.built === '0') {
    _buildPanelContent(tab, panel);
    if(!['admin','hacker','profile','chat'].includes(id)) logActivity('TAB', tab.name);
    return;
  }

  // ── Already built: do a targeted refresh ─────────────────────────
  if(tab.isFrontPage)  refreshFrontPagePanel();
  if(tab.isLoreHub)    refreshLoreHubPanel();
  ...
  if(tab.isSysNews)    panel.innerHTML = buildSysNewsPanel(tab);
  if(tab.isMarket)     refreshMarketPanel(tab);
  if(tab.isIdent)      refreshIdentPanel(tab);
  if(tab.isWanted)     refreshWantedPanel(tab);
  if(tab.isHub)        refreshHubPanel(tab);
